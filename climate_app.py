...
def precipitation():
    ...
    precip = session.query(measurement.date, meaurement.prcp).all()
        
    session.close()

    ##all_precip = list(np.ravel(precip)) - may need to jsonify this variable
...
# def end():
    session =Session(engine)
     ...
     T_end = session.query(tmin[(date >= 20100101) & (date <= 20101231)], tavg[(date >= 20100101) & (date <= 20101231)], tmax[(date >= 20100101) & (date <= 20101231)])
